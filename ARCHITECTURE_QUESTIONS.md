...
- **Gemini 2.5 Flash LLM (provider-pluggable).** The voice turn blocks on LLM time-to-first-token, so the default is a fast tool-calling tier rather than the most capable model — a latency decision. The provider is auto-detected from the configured key, so the same code runs `gpt-4o-mini` or `claude-haiku-4-5` (latency-equivalent) or a top-tier model for non-realtime workloads via one env var.
...
| LLM (gemini-2.5-flash) | ~50k input (~$0.30/MTok) + ~2k output (~$2.50/MTok), history resent per turn | ~$0.02 (gpt-4o-mini ~$0.01; claude-haiku-4-5 ~$0.06) |
